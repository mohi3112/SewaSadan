...
<section class="content">
    <div class="body_scroll">
        <div class="block-header"><div class="row"><div class="col-lg-7 col-md-6 col-sm-12"><h2>Total Security</h2></div></div></div>
            ...
            <div class="row clearfix">
                <div class="col-lg-12">
                    <div class="card">
                        <div class="body">
						<?php  // Code Here/////  ?>
					<table width="auto" cellspacing="6px" style="padding-top:100px;background-color:#ccc;" align="center" width="100%" bgcolor="#ccc">
						  <form method="get" action="totalsecurity.php">
						  <td>Start Slip No: <input type="text" name="ss"></td><td>End Slip No: <input type="text" name="ee"></td><td><input type="submit" name="chk" value="search"></td>
						  </tr>
						  </table> 
...
if(isset($_GET['chk']))
{
	...
	//	$ss=$fnyear."/".$start;
	//	$ee=$fnyear."/".$end;
$stq="SELECT * from `cash` where  `slip` = '$ss';";
 $strs=mysqli_query($con,$stq);
 $strset=mysqli_fetch_array($strs);	
$stpq="SELECT * from `cash` where  `slip` = '$ee';";
...
$tyr="SELECT sum(donation) as total from `cash` where `slip`!='0' and `sr` between '$starting' and '$ending';";
 $rtr=mysqli_query($con,$tyr);
 $totaladvance=mysqli_fetch_array($rtr);
 echo "<h1 align='center'>Total Security You Recived ".$totaladvance['total']." </h1>";
$qr="SELECT * from `cash` where `slip`!='0' and `sr` between '$starting' and '$ending';";
